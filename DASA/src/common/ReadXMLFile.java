...
public class ReadXMLFile {
	...
	public static void load() {
		...
		folder.add(new File(Constants.ANSWERS_FOLDERNAME));
		folder.add(new File(Constants.CORPUSPath_FOLDERNAME));
		folder.add(new File(Constants.QUESTIONS_FOLDERNAME));
		...
		for (int count = 0; count < nodeList.getLength(); count++) {
			...
			if (tempNode.getNodeType() == Node.ELEMENT_NODE) {
				if (tempNode.getNodeName().equals("doc")) {
					...
					try {
						writer = new BufferedWriter(new OutputStreamWriter(
								new FileOutputStream(Constants.QA4MRE_FOLDER
										+ "docs/" + d_id + ".txt"), "utf-8"));
				...
				if (tempNode.getNodeName().equals("q_str")) {
					// get doc_id
					Node node = tempNode.getParentNode().getParentNode()
				...
				if (tempNode.getNodeName().equals("answer")) {
					...
					if (tempNode.getTextContent().toUpperCase()
							.equals(Constants.DEFAULT_ANSWER) == false) {
						// write each answer content to a file
						Writer writer = null;
						try {
							writer = new BufferedWriter(new OutputStreamWriter(
									new FileOutputStream(
											Constants.QA4MRE_FOLDER
													+ "answers/" + d_id + "_"
													+ q_id + "_" + a_id
													+ ".txt"), "utf-8"));
							writer.write(tempNode.getTextContent());
